...
final class ClipboardXPCServer: NSObject, ClipboardXPCProtocol, NSXPCListenerDelegate {
    ...
    func listener(_ listener: NSXPCListener, shouldAcceptNewConnection newConnection: NSXPCConnection) -> Bool {
        guard Self.isAllowedClient(newConnection) else {
            container.log.warning("Rejected ClipboardXPC client pid=\(newConnection.processIdentifier)")
            return false
        }
        let iface = NSXPCInterface(with: ClipboardXPCProtocol.self)
    ...
    static func isAllowedClient(_ connection: NSXPCConnection) -> Bool {
        // Validate identity before exposing clipboard contents. Prefer an audit-token
        // based SecCode check when available; fall back to pid only for local Debug
        // builds. Do not accept arbitrary local processes on this Mach service.
    ...
    func paste(itemID: String, plainText: Bool, reply: @escaping (String) -> Void) {
        ...
        DispatchQueue.main.async {
            if plainText {
                if let pasteText = Self.plainText(from: body) {
            ...
            let result = Platform.PasteInjector.paste(nil, mode: plainText ? .plainText : .formatted)
