...
**Tech Stack:** `Foundation.Process`, `Network.framework` (`NWListener`), `Security` (Keychain for browser keys), `CommonCrypto` (macOS Chromium AES-CBC/PBKDF2 cookie decryption), `GRDB` (already in Plan 1) for reading browsers' cookie SQLite, `CryptoKit` for downloader-update signature verification (Ed25519), `AppKit` (`NSDockTile`), SwiftUI for views.

**Reads from spec:** §8 (entire), §3 (decisions 8, 10, 11), §10 (Hardened Runtime entitlements), §11 (telemetry hooks).

**Depends on:** Plan 0 (main app target, bundle structure), Plan 1 (`DownloadStore`, `KeyManager`, `Logging`).
...
public actor DownloadQueue {
    ...
    public init(maxConcurrent: Int, started: @escaping StartHandler, progress: @escaping ProgressHandler, completion: @escaping CompletionHandler) {
...
final class DownloadQueueTests: XCTestCase {
    func testCancelQueuedJobCompletesWithoutStartingProcess() async throws {
        ...
        let queue = DownloadQueue(maxConcurrent: 0, started: { _ in XCTFail("queued job must not start") }, progress: { _, _ in }) { completedID, result in
...
public enum ChromiumCookies {
    ...
    private static func decryptChromiumValue(_ encrypted: Data, safeStorageKey: Data) throws -> String {
        // macOS Chromium cookies use AES-128-CBC with key =
        // PBKDF2-HMAC-SHA1(password: Chrome Safe Storage, salt: "saltysalt",
        // iterations: 1003), IV = 16 spaces, and usually a "v10" prefix.
...
Add `CommonCrypto` to the Platform target linker settings if SwiftPM/Xcode does not import it transitively. Add tests with a fixed macOS Chromium vector: plaintext cookie value → AES-CBC/PBKDF2 encrypted `v10` blob → `decryptChromiumValue` returns the original value. Do not emit placeholder cookie values; if decryption fails, surface `ChromiumCookiesError.decryptFailed` and run the downloader without cookies for that profile.
